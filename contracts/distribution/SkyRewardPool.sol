...
contract SkyRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accSkyPerShare; // Accumulated SDSs per share, times 1e18. See below.
    ...
    // The timestamp when SDS mining starts.
    uint256 public startTime;
    uint256 public endTime;

    uint256 public nextHalvingTime;
    uint256 public rewardHalvingRate = 9850;
    ...
    constructor(
        ...
        endTime = _startTime.add(81 weeks);
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SdsRewardPool: caller is not the operator");
        _;
    }
        ...
        if (halvingChecked) {
            ...
            if (now >= endTime) {
                ...
                if (now >= nextHalvingTime) {
                    massUpdatePools();
                    rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(10000); // x98.5% (1.5% decreased every week)
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _lastRewardTime, uint256 _lockedTime) public onlyOperator {
    ...
    // Update the given pool's SDS allocation point. Can only be called by the owner.
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (now > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (totalAllocPoint > 0) {
                uint256 _shareReward = _time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
                accSkyPerShare = accSkyPerShare.add(_shareReward.mul(1e18).div(lpSupply));
            }
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _time = now.sub(pool.lastRewardTime);
            uint256 _shareReward = _time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accSkyPerShare = pool.accSkyPerShare.add(_shareReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = now;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (user.amount > 0) {
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _claimableAmount = user.amount.mul(pool.accSkyPerShare).div(1e18).sub(user.rewardDebt);
            if (_claimableAmount > 0) {
                IBasisAsset(share).mint(_account, _claimableAmount);
